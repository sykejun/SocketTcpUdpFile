...
import com.example.kejun.myapplication.socket.TcpServer;
import com.example.kejun.myapplication.socket.UdpServer;
...
public class ReceiveActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView = findViewById(R.id.receivefileListView);
        startRecvBtn = findViewById(R.id.startRecvBtn);
    ...
    class ReceiveFileTask extends AsyncTask<String, Integer, Integer> {
        ...
        protected void onPreExecute() {
            ...
            progressDialog.setCancelable(true); //用了这个方法之后，直到图片下载完成，进度条才会消失（即使在这之前点击了屏幕）
            progressDialog.show();
        }
        ...
        protected Integer doInBackground(String... params) {
            ...
            DatagramPacket senderPacket = udpServer.waitClient();//主要用于告诉客户端，我的IP地址
            udpServer.close();//udp技术相应完了，下面开始采用tcp技术获取数据
            if (senderPacket == null) {
                return -1;//超时得到null
            }

            senderIP = senderPacket.getAddress().getHostName();//获取发送者的ip
            Log.i(TAG,"发送信息端的IP:"+senderIP);
            ...
            //等待与发送方建立Tcp连接
            files = tcpServer.waitSenderConnect();//收到文件的描述信息
            ...
            publishProgress(-1, 0, 0);//文件发送可能失败   //调用publishProgress公布进度,最后onProgressUpdate方法将被执行
